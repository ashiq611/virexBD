...
const Portfolio = () => {
  ...
  return (
    <div className="container mx-auto">
      <div className="justify-center pt-28 lg:pt-0">
        <h1 className="text-4xl text-center font-bold text-white">
          Our Successful <span className="text-[#E547FF]">Projects!</span>
        </h1>
        <div className="flex gap-8 m-5 justify-center text-white">
          <button
            value=""
            onClick={() => setSearchQuery("")}
            className="text-[#E547FF]"
          >
            All
          </button>
          <button
            value="Ecommerce Solutions"
            onClick={() => setSearchQuery("Ecommerce Solutions")}
          >
            Website
          </button>
          <button
            value="Mobile App"
            onClick={() => setSearchQuery("Mobile App")}
          >
            Application
          </button>
        </div>
      ...
      <div className="grid grid-cols-2 gap-10 m-12 ">
        {projects.map((data) => (
          <Projects key={data._id} data={data}></Projects>
        ))}
      </div>
    </div>
  );
